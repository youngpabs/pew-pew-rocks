...
bool Toggler::DownHit(double xMouse, double yMouse, int state, int proximity) {
	return DownHit((int) xMouse, (int) yMouse, state, proximity);
}

bool Toggler::DownHit(int xMouse, int yMouse, int state, int proximity) {
	bool down = state == GLFW_PRESS, hit = Hit((int) xMouse, (int) yMouse, proximity);
	if (down && hit && ptr != NULL)
		*ptr = *ptr? false : true;
	return down && hit;
}
