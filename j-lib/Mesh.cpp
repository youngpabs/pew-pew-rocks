...
void Mesh::Display(CameraAB camera) {
	...
	if (!nPts || !nTris) return;
	// enable shader and vertex array object
	int shader = UseMeshShader();
	glBindVertexArray(vao);
	// texture
	SetUniform(shader, "useTexture", textureUnit && nUvs);
	if (nUvs && textureUnit) {
...
bool Mesh::Read(string objFile, mat4 *m) {
	if (!ReadAsciiObj((char *) objFile.c_str(), points, triangles, &normals, &uvs)) {
...
bool Mesh::Read(string objFile, string texFile, int texUnit, mat4 *m) {
	if (!Read(objFile, m))
		return false;
	if (!texUnit) {
